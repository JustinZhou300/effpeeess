...
func throw_held():
	...
	if thrown_obj.is_melee: #rotates weapon to point forward if knife etc.
		thrown_obj.look_at(biped.shoot_ray.get_collision_point())
		thrown_obj.rotation += Vector3(-90, 0, 0)
